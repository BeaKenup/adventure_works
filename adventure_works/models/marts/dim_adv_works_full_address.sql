...
With
    address as (
        select *
        from {{ ref('stg_adv_works_address')}}
    )
    , state_province as (
        select *
        from {{ ref('stg_adv_works_state_province') }}
    )
    , country_region as (
        select *
        from {{ ref('stg_adv_works_country_region') }}
    )
...
-- Acrescentando a stg_sdv_works_country_region utilizando country_region_code
    , final as (
        select
            ...
            , address_with_state_province.is_only_state_province_flag 
